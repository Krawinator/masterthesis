    ...
    return [
        ...
        },
        {
            ...
            "style": {
                "curve-style": "bezier",
                "label": "data(label)",
                "text-opacity": 1 if show_edge_labels else 0,
                "font-size": 10,
            },
        },
...
def terms_to_string(terms):
    ...
    if not terms:
        return ""
    return ", ".join(
        f"{t.get('node','')}:{t.get('coeff','')}" for t in terms if t.get("node") is not None
    )
            ...
            [
                ...
                html.Div(
                    [
                        ...
                        html.Div(
                            [
                                dcc.Checklist(
                                    ...
                                    options=[
                                        {
                                            "label": "Kanten-Beschriftungen anzeigen",
                                            "value": "show",
                                        }
                                    ],
                                    value=[],
                        ...
                        html.Button(
                            "Ausgewählten Node/Edge löschen",
                            id="btn_del_element",
                        ...
                        html.Div(
                            [
                                ...
                                html.Button(
                                    "Edge hinzufügen (2 Nodes wählen)",
                                    id="btn_add_edge",
                                    style={"marginLeft": 6},
                                ),
                            ]
                        ...
                        html.Div(
                            [
                                ...
                                dcc.Checklist(
                                    ...
                                    options=[
                                        {
                                            "label": "Dieses Feld wird aus anderen Feldern abgeleitet",
                                            "value": "on",
                                        }
                                    ],
                                    value=[],
                        ...
                        html.Button(
                            "Node-Felder speichern",
                            id="btn_save_node_schema",
                            style={"marginTop": 6},
                        ),
                        # ===== Edge-Felder =====
                        html.H5("Edge-Felder"),
                        html.Div(
                            [
                                html.Label("Strom_Limit_in_A"),
                                dcc.Input(
                                    id="e_Strom_Limit_in_A", type="number"
                                ),
                                html.Label("Reaktanz X_total (Ω)"),
                                dcc.Input(
                                    id="e_X_total_ohm", type="number", step="any"
                                ),
                            ],
                            style={
                                "display": "grid",
...
def show_selection(node_data, edge_data):
    ...
    if node_data:
        ...
        return (
            ...
            None,  # e_Strom_Limit_in_A
            d["id"],
            d.get("label"),
            "",
            "",
            feats.get("Latitude_deg"),
            feats.get("Longitude_deg"),
            None,  # e_X_total_ohm
            feats.get("busbar_id", ""),
            d_enabled,
            d_feature,
    ...
    if edge_data:
        ...
        feats = d.get("features", {}) or {}
        summary = (
            f"[Edge] id={d.get('id')} {d.get('source')}→{d.get('target')}\n"
            f"{json.dumps(feats, indent=2)}"
        )
        return (
            summary,
            "",  # n_P
...
def mutate_graph(
    ...
    def split(el):
        nodes_ = [x for x in el if "source" not in x.get("data", {})]
        edges_ = [x for x in el if "source" in x.get("data", {})]
    ...
    # --- Element löschen (Node ODER Edge) ---
    if trig == "btn_del_element":
        # 1) Node ausgewählt → Node + alle zugehörigen Edges löschen
        if selected_nodes:
            nid = selected_nodes[0]["id"]
            nodes = [n for n in nodes if n["data"]["id"] != nid]
            edges = [
                e for e in edges
                if e["data"].get("source") != nid and e["data"].get("target") != nid
            ]
            return nodes + edges, nodes + edges

        # 2) Edge ausgewählt → nur diese Edge löschen
        if selected_edges:
            ed = selected_edges[0]
            eid = ed.get("id")
            s = ed.get("source")
            t = ed.get("target")
            if eid is not None:
                edges = [e for e in edges if e["data"].get("id") != eid]
            else:
                edges = [
                    e for e in edges
                    if not (
                        e["data"].get("source") == s and e["data"].get("target") == t
                    )
                ]
            return nodes + edges, nodes + edges

        # Nichts ausgewählt – nichts tun
        return nodes + edges, nodes + edges
    ...
    if trig == "btn_save_node_schema" and selected_nodes:
        ...
        for n in nodes:
            if n["data"]["id"] == nid:
                ...
                # Standardfelder
                if n_P not in (None, ""):
                    feats["P_Datapoint_ID"] = n_P
                if n_Ilim not in (None, ""):
                    feats["Strom_Limit_in_A"] = float(n_Ilim)
                if n_DAB not in (None, ""):
                    feats["DAB_ID"] = n_DAB
                if n_type_sel:
                    n["data"]["type"] = n_type_sel
                if n_pmax_mw not in (None, ""):
                    feats["p_max_MW"] = float(n_pmax_mw)
                else:
                    feats.pop("p_max_MW", None)
                if n_lat not in (None, ""):
                    feats["Latitude_deg"] = float(n_lat)
                if n_lon not in (None, ""):
                    feats["Longitude_deg"] = float(n_lon)

                # Busbar-Formel als String
    ...
    if trig == "btn_save_edge_schema" and selected_edges:
        ...
        if target_edge:
            ...
            if e_Ilim not in (None, ""):
                feats["Strom_Limit_in_A"] = float(e_Ilim)
            if e_X_total not in (None, ""):
                feats["X_total_ohm"] = float(e_X_total)
    ...
    if trig == "upload_json":
        if upload_contents and upload_name and upload_name.endswith(".json"):
            try:
                decoded = base64.b64decode(upload_contents.split(",")[1]).decode(
                    "utf-8"
                )
